...
// ============================================
const pendingUsers = new Map(); // Stores users waiting for verification
const verificationCodes = new Map(); // Stores OTP/tokens with expiry
...
async function sendVerificationLinkEmail(email, token) {
...
app.post('/signup-with-otp', async (req, res) => {
    try {
        ...
        if (pendingUsers.has(email)) {
            // Allow re-sending if they try again? For now just keep it simple
        }

        // Generate OTP
        const otp = generateOTP();

        // Store user data temporarily (include extra fields for Supabase profile)
        pendingUsers.set(email, {
            email,
            password: password, // Store original password for Supabase creation
            fullName,
            rollNo,
            regulation,
            createdAt: Date.now(),
...
// Route 2: User Signup (with Verification Link)
...
async function finalizeUserCreation(email) {
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
        ...
        email_confirm: true // Mark as confirmed since we verified them
    });
...
app.post('/forgot-password', async (req, res) => {
    try {
        ...
        // Store OTP with prefix to distinguish from signup OTP
...
app.post('/reset-password', async (req, res) => {
    try {
        ...
        // 1. Get User ID by email using Admin SDK
